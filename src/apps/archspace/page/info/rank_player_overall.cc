...
bool
CPageRankPlayerOverall::handler(CPlayer *aPlayer)
{
//	system_log( "start page handler %s", get_name());
	...
	ITEM("TOP_TEN_RANK_NAME", GETTEXT("Overall Leading Players"));
	ITEM("TOP_TEN_RANK",
			RankTable->top_ten_info_html(aPlayer->get_game_id()));

	ITEM("AROUND_ME_RANK_NAME",
			(char *)format(GETTEXT("Players With Similar Overall Power Near %1$s"),
							aPlayer->get_nick()));
	ITEM("AROUND_ME_RANK",
