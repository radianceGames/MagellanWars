...
bool
CPageRankCouncilDiplomatic::handler(CPlayer *aPlayer)
{
//	system_log( "start page handler %s", get_name());

	CRankTable *
		RankTable = COUNCIL_TABLE->get_diplomatic_rank_table();
	CCouncil *
	...
	ITEM("AROUND_ME_RANK_NAME",
			(char *)format(GETTEXT("Councils With Similar Diplomatic Influence Near %1$s"),
							Council->get_nick()));
	ITEM("AROUND_ME_RANK",
