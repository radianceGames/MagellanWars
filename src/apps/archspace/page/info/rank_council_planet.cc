...
bool
CPageRankCouncilPlanet::handler(CPlayer *aPlayer)
{
//	system_log( "start page handler %s", get_name());

	CRankTable *
		RankTable = COUNCIL_TABLE->get_planet_rank_table();
	CCouncil *
	...
	ITEM("AROUND_ME_RANK_NAME",
			(char *)format(GETTEXT("Councils With Similar Planet Power Near %1$s"),
							Council->get_nick()));
	ITEM("AROUND_ME_RANK",
