...
bool
CPageRankPlayerPlanet::handler(CPlayer *aPlayer)
{
//	system_log( "start page handler %s", get_name());

	CRankTable *
		RankTable = PLAYER_TABLE->get_planet_rank_table();
	...
	ITEM("AROUND_ME_RANK_NAME",
			(char *)format(GETTEXT("Players With Similar Planet Power Near %1$s"),
							aPlayer->get_nick()));
	ITEM("AROUND_ME_RANK",
