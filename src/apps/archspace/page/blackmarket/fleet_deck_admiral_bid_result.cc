...
bool
CPageBlackMarketFleetDeckAdmiralBidResult::handler(CPlayer *aPlayer)
{
	...
	aPlayer->get_admiral_pool()->remove_without_free_admiral( admiral->get_id() );
	fleet->set_admiral( admiral->get_id() );
	admiral->set_owner( 0 );	// black market
	...
	*STORE_CENTER << *fleet;
	...
	return output("black_market/fleet_deck_admiral_bid_result.html");
}
